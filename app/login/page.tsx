...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(['admin', 'super_admin'].includes(data.role) ? '/admin/dashboard' : '/educator/dashboard')
    } catch {
      ...
      <div className="w-full max-w-[400px] relative">
        ...
        <div className="bg-white/[0.04] backdrop-blur-sm border border-white/[0.08] rounded-2xl p-7 shadow-2xl">
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            {/* Email */}
            <div>
              <label className="text-[11.5px] font-bold text-slate-400 uppercase tracking-wider block mb-1.5">
                Email Address
              </label>
            ...
            <button type="submit" disabled={loading}
              className="w-full h-11 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold text-[13.5px] rounded-xl hover:shadow-xl hover:shadow-amber-500/25 hover:-translate-y-0.5 active:translate-y-0 transition-all disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-none">
              {loading ? (
        ...
        <p className="text-center mt-6 text-[11px] text-slate-600">
          © {new Date().getFullYear()} Whyte Pyramid Academy · Staff Portal
        </p>
      </div>
    </div>
  )
